...
class DDPM(pl.LightningModule):
    ...
    def predict_noise_from_start(self, x_t, t, x_0):
        return (extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t - x_0) / extract_into_tensor(
...
class LatentDiffusion(DDPM):
    ...
    def on_train_start(self):
        """Ensure the config is logged to wandb"""
        with open(os.path.join(self.logdir, "configs", "lightning.yaml"), "r") as file:
            lightning_config = yaml.safe_load(file)
            self.logger.experiment.config.update(lightning_config, allow_val_change=True)

        with open(os.path.join(self.logdir, "configs", "model.yaml"), "r") as file:
            model_config = yaml.safe_load(file)
            self.logger.experiment.config.update(model_config, allow_val_change=True)
    ...
    def shared_step(self, batch, random_uncond, **kwargs):
        ...
        loss, loss_dict, info = self(x, c, **kwargs)
    ...
    def model_predictions(self, x_noisy, t, cond, **kwargs):
        model_output, _ = self.apply_model(x_noisy, t, cond, **kwargs)

        if self.parameterization == "eps":
            pred_noise = model_output
            x_start = self.predict_start_from_noise(x_noisy, t, pred_noise)
            pred_v = self.get_v(x_start, pred_noise, t)

        elif self.parameterization == "x0":
            x_start = model_output
            pred_noise = self.predict_noise_from_start(x_noisy, t, x_start)
            pred_v = self.get_v(x_start, pred_noise, t)

        elif self.parameterization == "v":
            pred_v = model_output
            x_start = self.predict_start_from_z_and_v(x_noisy, t, pred_v)
            pred_noise = self.predict_eps_from_z_and_v(x_noisy, t, pred_v)
    ...
    def training_step(self, batch, batch_idx):
        loss, loss_dict, info = self.shared_step(batch, random_uncond=self.classifier_free_guidance)
        self.log_dict(loss_dict, prog_bar=True, logger=True, on_step=True, on_epoch=True, sync_dist=False)
        info = {f"train/{key}": value for key, value in info.items()}
        self.log_dict(info, prog_bar=False, logger=True, on_step=True, on_epoch=True, sync_dist=False)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with self.ema_scope():
            _, loss_dict, info = self.shared_step(batch, random_uncond=self.classifier_free_guidance)
        self.log_dict(loss_dict, prog_bar=False, logger=True, on_step=False, on_epoch=True)
        info = {f"val/{key}": value for key, value in info.items()}
        self.log_dict(info, prog_bar=False, logger=True, on_step=False, on_epoch=True)
    ...
    def log_images(
        ...
        **kwargs,
        ...
        z, c, xrec, xc = self.get_batch_input(
            batch, random_uncond=False, return_first_stage_outputs=True, return_original_cond=True
        )
    ...
    def p_mean_variance(
        ...
        t_in = t
        model_out, _ = self.apply_model(x, t_in, c, **kwargs)
...
class LatentVisualDiffusion(LatentDiffusion):
    def __init__(
        ...
        **kwargs,
        ...
        self.save_only_unet = save_only_unet
    ...
    def on_save_checkpoint(self, checkpoint):
        if self.save_only_unet:
            model_state_dict = {
                k: v for k, v in self.state_dict().items() if (k.startswith("model") or k.startswith("ema_model"))
            }
    ...
    def get_batch_input(
        ...
        **kwargs,
        ...
        input_mask = 1 - rearrange(
            (random_num >= self.uncond_prob).float() * (random_num < 3 * self.uncond_prob).float(), "n -> n 1 1 1"
        )
    ...
    def prepare_batch_for_inference(self, batch):
        kwargs = {}
        log = {}
        z, c, _, xc, fs, cond_x, x = self.get_batch_input(
            batch,
            random_uncond=False,
        ...
        cond_mask = torch.zeros_like(z)
        cond_mask[:, :, 0, :, :] = 1.0
        cond_mask.to(self.device)
        ...
        return z, c, uc, cond_mask, log, kwargs
    ...
    def log_images(
        ...
        **kwargs,
        ...
        use_ddim = ddim_steps is not None
        z, c, uc, cond_mask, log, new_kwargs = self.prepare_batch_for_inference(batch)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.conditioning_key == "hybrid":
            ...
            out = self.diffusion_model(xc, t, context=cc, act=act, dropout_actions=dropout_actions, **kwargs)
        else:
            raise NotImplementedError()

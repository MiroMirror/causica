...
def get_dataset_statistics(
    ...
    if save_dir is not None:
        path = os.path.join(save_dir, f"dataset_statistics_{unique_hash}.json")
    else:
        path = local_path
...
def combine_dataset_statistics(
    ...
    for key in merge_stat_keys:
        combined_mean = np.array([stat[key]["mean"] * w for stat, w in zip(all_dataset_statistics, stat_weights)]).sum(
            0
        )
        ...
        combined_dataset_statistics[key] = {
            "min": np.array([stat[key]["min"] for stat in all_dataset_statistics]).min(0).tolist(),
            "max": np.array([stat[key]["max"] for stat in all_dataset_statistics]).max(0).tolist(),
...
def normalize_action_and_proprio(traj: dict, metadata: dict, normalization_type: NormalizationType, skip_keys=None):
        ...
        for key, traj_key in keys_to_normalize.items():
            ...
            traj = dl.transforms.selective_tree_map(
                ...
                map_fn=lambda x: tf.where(
                    mask,
                    tf.clip_by_value(
                        2 * (x - metadata[key]["min"]) / (metadata[key]["max"] - metadata[key]["min"] + 1e-8) - 1,
                        -1,
                        1,
                    ),
                    x,
